...
// TextInput is the Bubble Tea model for this text input element.
...
func (m TextInput) Cursor() int {
    return m.pos
}

// Blink returns whether or not to draw the cursor.
func (m TextInput) Blink() bool {
    return m.blink
}
...
func (m *TextInput) setCursor(pos int) bool {
...
func (m *TextInput) SetCursorMode(mode CursorMode) tea.Cmd {
    m.cursorMode = mode
...
func (m *TextInput) cursorEnd() bool {
...
func (m *TextInput) handlePaste(v string) bool {
    paste := []rune(v)
...
func (m *TextInput) deleteBeforeCursor() bool {
...
func (m *TextInput) deleteAfterCursor() bool {
    m.value = m.value[:m.pos]
    return m.setCursor(len(m.value))
}
...
func (m *TextInput) deleteWordLeft() bool {
...
// the cursor blink should be reset. If input is masked delete everything after
// the cursor so as not to reveal word breaks in the masked input.
func (m *TextInput) deleteWordRight() bool {
...
func (m *TextInput) wordLeft() bool {
    if m.pos == 0 || len(m.value) == 0 {
        return false
    }
...
// so as not to reveal word breaks in the masked input.
func (m *TextInput) wordRight() bool {
    if m.pos >= len(m.value) || len(m.value) == 0 {
        return false
    }
...
func (m TextInput) echoTransform(v string) string {
    switch m.EchoMode {
    case EchoPassword:
    ...
    case tea.KeyMsg:
        ...
        case tea.KeyUp:
            ...
            if m.cmdHistoryPos < 0 {
                m.cmdHistoryPos = len(m.cmdHistory) - 1
            }

            if len(m.cmdHistory) > 0 && m.cmdHistoryPos < len(m.cmdHistory) {
                m.value = []rune(m.cmdHistory[m.cmdHistoryPos])
                m.setCursor(0)
            }
        ...
        case tea.KeyDown:
            ...
            if m.cmdHistoryPos < len(m.cmdHistory) {
                m.value = []rune(m.cmdHistory[m.cmdHistoryPos])
                m.setCursor(0)
            }
            break
        case tea.KeyEnter:
            if len(m.value) != 0 {
                m.cmdHistory = append(m.cmdHistory, string(m.value))
                m.setCursor(0)

                if m.InputHandler != nil {
                    m.InputHandler(string(m.value))
                }
            }
            break
...
func (m TextInput) cursorView(v string) string {
    if m.blink {
...
func (m *TextInput) blinkCmd() tea.Cmd {
    if m.cursorMode != CursorBlink {
        return nil
    }
